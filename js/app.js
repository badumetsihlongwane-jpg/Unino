...
function renderStoriesRow(hostId = 'stories-row', options = {}) {
  ...
  db.collection('stories').where('expiresAt', '>', cutoff).orderBy('expiresAt', 'desc').limit(50).get().then(snap => {
    const stories = snap.docs.map(d => ({ id: d.id, ...d.data() }));
    const grouped = {};
    stories.forEach(story => {
      if (!story.authorId) return;
      if (story.authorId !== state.user.uid && !myFriends.has(story.authorId)) return;
  ...
  try {
    ...
    body.innerHTML = posts.length ? posts.map(p => `
      <div class="asg-card" style="margin:8px 0">
        ...
        <div class="asg-card-meta" style="margin-top:6px;flex-wrap:wrap">
          <span>Reports: ${p.reportsCount || 0}</span>
          <span>· Review: ${esc(p.reviewState || 'clear')}</span>
          ${p.softFilterKeyword ? `<span>· Keyword: ${esc(p.softFilterKeyword)}</span>` : ''}
          ${p.shadowBanned ? '<span>· Shadow banned</span>' : ''}
        </div>
